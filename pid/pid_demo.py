...
# 以车辆后轴中心为中心的车辆运动学模型的PID控制演示demo
def vehicle_pid_main():
    # 时间范围
    T = 100
    dt = 0.1
    num_steps = int(T / dt)
    ...
    pid = PID(upper = np.pi / 2, lower = -np.pi / 2, k = [0.6, 0.004, 1.0])
    ...
    for i in range(num_steps):
        ...
        # PID
        pos_delta = pre_aim_point - np.array([vehicle.x_cartesian, vehicle.y_cartesian])
        l_d = np.linalg.norm(pos_delta)
        alpha = np.arctan2(pos_delta[1], pos_delta[0])
        theta_e = alpha - vehicle.psi
        ey = -l_d * np.sin(theta_e) # 因为要让模型往左打方向delta_f是负，往右大方向delta_f是正
        delta_f = pid.cal_output(0.0, ey)
